#!/usr/bin/env bash
# Idempotent v3 base installation. Re-running preserves newer managed tools.
set -Eeuo pipefail
...
if (( node_ready == 0 )); then
  download_dir="$(mktemp -d /tmp/paseo-v3-download.XXXXXX)"
...
# Reinstall only when the checked-in lockfile changed or node_modules is incomplete.
...
if [[ ! -x "$PASEO_ROOT/uv/bin/uv" ]]; then
  ...
  "$PASEO_ROOT/uv/bin/pip" install --disable-pip-version-check --no-input 'uv==0.12.11'
else
...
chmod 0700 /srv/paseo
for directory in .codex .paseo projects worktrees tools tools/bin cache cache/tmp; do
...
printf 'paseo-debian-v3 install complete\n' > "$PASEO_ROOT/.install-complete"
