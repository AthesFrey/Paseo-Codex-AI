...
工作区根目录是 /srv/paseo/projects；任务 worktree 位于 /srv/paseo/worktrees。现有仓库先检查 git status，保留用户未提交改动；较大的独立任务使用新的 codex/<任务名> 分支或 worktree。普通问题不需要创建分支，不自动合并、推送或部署生产环境。
