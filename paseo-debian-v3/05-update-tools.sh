#!/usr/bin/env bash
# Intentionally aggressive v3 tool updater. It updates only the v3-managed paths.
set -Eeuo pipefail
...
if [[ "${update_node:-0}" == 1 ]]; then
  ...
  node_index="$(mktemp /tmp/paseo-v3-node-index.XXXXXX)"
  node_download="$(mktemp -d /tmp/paseo-v3-node-download.XXXXXX)"
...
if (( node_updated == 1 )) || [[ "${update_paseo:-0}" == 1 || "${update_codex:-0}" == 1 ]]; then
  npm_args=(--prefix "$PASEO_ROOT/apps" install --save-exact --force --no-audit --no-fund)
  ...
  "$PASEO_ROOT/node/bin/npm" ci --prefix "$PASEO_ROOT/apps" --omit=dev --force --no-audit --no-fund
...
if [[ "${update_uv:-0}" == 1 ]]; then
  "$PASEO_ROOT/uv/bin/python" -m pip install --upgrade --force-reinstall --no-cache-dir uv
fi
