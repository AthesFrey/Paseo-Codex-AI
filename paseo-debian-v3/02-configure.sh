#!/usr/bin/env bash
# Idempotent v3 configuration. Existing key/password/config are preserved.
set -Eeuo pipefail
...
for path in /srv/paseo/.codex/config.toml /srv/paseo/.paseo/config.json \
  /srv/paseo/AGENTS.md /srv/paseo/.profile \
...
[[ -e /srv/paseo/AGENTS.md ]] || install -o paseo -g paseo -m 0644 "$PASEO_KIT/templates/AGENTS.md" /srv/paseo/AGENTS.md
...
lines = Path('/etc/paseo-v3/runtime.env').read_text().splitlines()
profile = ['# Paseo v3 project environment; credentials are intentionally absent.']
for line in lines:
...
path = Path('/etc/paseo-v3/hahaapi.env')
...
PY
  ...
  python3 - <<'PY'
from pathlib import Path
text = Path('/etc/paseo-v3/hahaapi.env').read_text()
...
# This unit is owned by v3; a rerun refreshes it to the package template.
