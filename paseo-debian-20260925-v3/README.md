# paseo-debian-20260925-v3：Debian 部署工具包
...
| `/srv/paseo/deploy-kit/paseo-debian-20260925-v3` | 从 GitHub 下载并保留的部署包源文件 |
...
将根目录 `install.sh` 与本目录一起提交。保持交互式 SSH 终端，在 VPS 上运行：

```bash
curl -fsSL https://raw.githubusercontent.com/AthesFrey/Paseo-Codex-AI/main/install.sh | sudo bash
```

`install.sh` 从 GitHub `main` 下载源码归档，从中取出 v3 工具包并校验包内 `SHA256SUMS`。Relay 检查和运行时安装成功后，脚本将工具包保存在 `/srv/paseo/deploy-kit/paseo-debian-20260925-v3`，再交互配置服务、配对客户端并执行最终验收。
...
```bash
sudo bash /srv/paseo/deploy-kit/paseo-debian-20260925-v3/01-install.sh --force
```
...
```bash
sudo bash /srv/paseo/deploy-kit/paseo-debian-20260925-v3/02-configure.sh
```
...
```bash
sudo bash /srv/paseo/deploy-kit/paseo-debian-20260925-v3/03-pair.sh
```
...
```bash
sudo bash /srv/paseo/deploy-kit/paseo-debian-20260925-v3/99-uninstall.sh --dry-run
```

执行卸载：

```bash
sudo bash /srv/paseo/deploy-kit/paseo-debian-20260925-v3/99-uninstall.sh --yes
```
