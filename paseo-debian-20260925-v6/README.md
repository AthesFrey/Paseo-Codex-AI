# paseo-debian-20260925-v6：Debian 部署工具包
...
| `/srv/paseo/deploy-kit/paseo-debian-20260925-v6` | 从 GitHub 下载并保留的部署包源文件 |
...
```bash
curl -fsSL https://raw.githubusercontent.com/AthesFrey/Paseo-Codex-AI/main/paseo-debian-20260925-v6/install.sh | sudo bash
```

`install.sh` 从 GitHub `main` 下载源码归档，从中取出 v6 工具包并校验包内 `SHA256SUMS`。Relay 检查和运行时安装成功后，脚本将工具包保存在 `/srv/paseo/deploy-kit/paseo-debian-20260925-v6`，再交互配置服务、配对客户端并执行最终验收。

如果目标机器已有本工具托管的运行时，需要明确使用强制重建入口：

```bash
curl -fsSL https://raw.githubusercontent.com/AthesFrey/Paseo-Codex-AI/main/paseo-debian-20260925-v6/install.sh | sudo bash -s -- --force
```

`--force` 会清除并重建托管运行时、`/etc/paseo`、Paseo/Codex 状态和 systemd unit，丢弃现有密钥、会话与配对身份；会保留 `/srv/proj`、项目文件、`worktrees`、`tools`、`cache` 和部署包目录。
...
如果工具包已经保存到部署目录，也可以直接重建当前 Paseo 托管部署：

```bash
sudo bash /srv/paseo/deploy-kit/paseo-debian-20260925-v6/01-install.sh --force
```
...
```bash
sudo bash /srv/paseo/deploy-kit/paseo-debian-20260925-v6/02-configure.sh
```
...
```bash
sudo bash /srv/paseo/deploy-kit/paseo-debian-20260925-v6/03-pair.sh
```
...
```bash
sudo bash /srv/paseo/deploy-kit/paseo-debian-20260925-v6/99-uninstall.sh --dry-run
```

执行卸载：

```bash
sudo bash /srv/paseo/deploy-kit/paseo-debian-20260925-v6/99-uninstall.sh --yes
```
