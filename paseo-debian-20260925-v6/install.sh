...
readonly RELEASE='paseo-debian-20260925-v6'
...
--force 将参数传递给 01-install.sh，停止并重建 Paseo 托管运行时、配置、密钥和应用状态。
USAGE
    exit 0
    ;;
  *)
...
require_root() {
  [[ $EUID == 0 ]] || { echo '请使用：curl .../paseo-debian-20260925-v6/install.sh | sudo bash' >&2; exit 1; }
}
