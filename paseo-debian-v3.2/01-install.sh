#!/usr/bin/env bash
# Idempotent v3.2 base installation. Re-running preserves newer managed tools.
set -Eeuo pipefail
...
if (( node_ready == 0 )); then
  download_dir="$(mktemp -d /tmp/paseo-v3.2-download.XXXXXX)"
...
printf 'paseo-debian-v3.2 install complete\n' > "$PASEO_ROOT/.install-complete"
