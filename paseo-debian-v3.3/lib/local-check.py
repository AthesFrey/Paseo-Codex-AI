...
import json, stat, subprocess, sys, time, urllib.request
...
def check_once():
    props = dict(line.split('=',1) for line in run('systemctl','--no-pager','show','paseo.service','-p','ActiveState','-p','User','-p','WorkingDirectory').splitlines())
    assert props.get('ActiveState') == 'active' and props.get('User') == 'paseo', '服务未以 paseo 用户 active 运行'
    assert props.get('WorkingDirectory') == '/srv/proj', '服务默认项目目录必须为 /srv/proj'
    ...
    if backapi.exists():
        backapi_stat = backapi.stat()
        assert backapi_stat.st_uid == 0 and backapi_stat.st_gid == 0 and stat.S_IMODE(backapi_stat.st_mode) == 0o600, 'BackAPI key 文件权限异常'
        ...
        assert provider is None, 'BackAPI provider 存在但 key 文件缺失'
