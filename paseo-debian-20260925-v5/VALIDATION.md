...
对象：`paseo-debian-20260925-v5/`、同名 ZIP 和包内 GitHub `install.sh`。依赖版本沿用 2026-09-25 的固定版本锁。

## 版本与依赖

- Node.js 26.10.0 / npm 11.19.1、`@getpaseo/cli@0.9.2`、`@openai/codex@0.157.0`、uv 0.12.19 保持不变。
- npm manifest 和 lockfile 根包版本为 `2026.9.25-v5`；安装记录和验收核对 `paseo-debian-20260925-v5`。
- v5 不包含安装验收后的缓存清理功能；运行时需要的 cache 环境变量和目录仍保留。

## 本次本地验证

- 包内 `install.sh` 和 v5 全部 shell 脚本通过 `bash -n`；脚本中的 Python heredoc 与 `lib/local-check.py` 可编译。
- Paseo JSON 模板、npm manifest/lockfile 可解析，Codex TOML 可解析；manifest 与 lockfile 的根版本、依赖一致，第三方依赖图未改变。
- 用本地构造的 GitHub 源码归档验证了 v5 目录的唯一发现、解包、`SHA256SUMS` 校验和关键文件检查；下载失败、缺少 v5 目录、校验失败均在安装阶段前停止。
